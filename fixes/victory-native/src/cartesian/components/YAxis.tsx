    ...
    return (
      <React.Fragment key={`y-tick-${tick}`}>
        ...
        {font && (
              <Text
                color={labelColor}
                text={tick == yTicksNormalized[yTicksNormalized.length - 1] ? contentY : tick == yTicksNormalized[0] ? contentY : ''}
                font={font}
                y={tick == yTicksNormalized[yTicksNormalized.length - 1] ? chartBounds.bottom - 20 : tick == yTicksNormalized[0] ? chartBounds.top + 30 : undefined}
                x={labelX - 5}
              />
            )}
      </React.Fragment>
